...
V5 = "/Users/dannyzweben/Desktop/ABCD-smartphon-socialmedia-review/claude-search-subrepo/v7_screening"
...
def main():
    ...
    # Merge in pre-tagged NA_FOR_NOW papers (no PDF)
    na_records = json.load(open(os.path.join(V5,"no_pdf_papers.json")))

    pids = sorted(meta, key=lambda x: int(x) if x.isdigit() else 999999)
    print(f"Total Stage-1 includes (PDFs): {len(pids)}")
    print(f"Pre-tagged NA_FOR_NOW (no PDFs): {len(na_records)}")

    rows = []
    counts = {"INCLUDE":0,"EXCLUDE":0,"UNSURE":0,"NA_FOR_NOW":0}
    ec_counts = {}

    # Pre-tagged NA_FOR_NOW papers
    for nr in na_records:
        row = {
            "paper_id": nr["paper_id"], "title": nr.get("title",""),
            "authors": nr.get("authors",""), "year": nr.get("year",""),
            "doi": nr.get("doi",""), "has_pdf": False,
            "FINAL_decision": "NA_FOR_NOW", "FINAL_EC_code": "",
            "FINAL_reason": "no PDF available; not scored",
        }
        for crit in CRITERIA:
            row[f"{crit}_C1"] = ""; row[f"{crit}_C2"] = ""
            row[f"{crit}_Resolver"] = ""; row[f"{crit}_FINAL"] = ""
            row[f"{crit}_C1_evidence"] = ""; row[f"{crit}_C2_evidence"] = ""
            row[f"{crit}_Resolver_rationale"] = ""
        rows.append(row); counts["NA_FOR_NOW"] += 1
